...
class GameScene extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image('spaceBg', 'assets/bgmenu.jpg');
        ...
        this.load.audio('astronautSound', 'assets/music/intro (1).m4a', { stream: true });
        this.load.audio('sunSound', 'assets/music/intro (2).m4a', { stream: true });
        this.load.audio('earthSound', 'assets/music/intro (4).m4a', { stream: true });
        this.load.audio('moonSound', 'assets/music/intro (3).m4a', { stream: true });
        this.load.audio('astronautSound2', 'assets/music/intro (5).m4a', { stream: true });
        this.load.audio('spaceBgm', 'assets/music/soundfx.mp3');
    }

    create() {
        this.add.image(window.innerWidth / 2, window.innerHeight / 2, 'spaceBg').setDisplaySize(window.innerWidth, window.innerHeight);
        ...
        this.time.delayedCall(30000, () => { // Delay 10 detik (sesuaikan jika perlu)
            this.spaceBgm = this.sound.add('spaceBgm', { loop: true, volume: 0.5 });
            this.spaceBgm.play();
        });
    }
    addPlanetLabels() {
    ...
    animateAstronaut() {
        let descriptions = [
            { obj: this.astronaut, text: 'HAII! Selamat Datang Di Simulasi Gerhana , Perkenalkan aku Aris , Aku Astronaut Yang Akan Memandu Kamu Mengenal Simulasi ini! ', sound: 'astronautSound' },
            { obj: this.sun, text: 'Ini adalah Matahari, sumber cahaya utama kita.', sound: 'sunSound' },
            { obj: this.earth, text: 'Ini adalah Bumi, tempat kita tinggal.', sound: 'earthSound' },
            { obj: this.moon, text: 'Ini adalah Bulan, satelit alami Bumi.', sound: 'moonSound' },
            { obj: this.astronaut, text: 'Sekarang! ayo kita bermain mencari letak posisi bagaimana gerhana bisa terjadi', sound: 'astronautSound2' },
        ];
        ...
        let highlightNext = () => {
            if (index < descriptions.length) {
                ...
                let sound = this.sound.add(current.sound);
                sound.play(); // Mainkan suara sesuai objek
    ...
    checkEclipse() {
        ...
        if (isSolarEclipse) {
            this.showEclipsePopup('Gerhana Matahari terjadi!', this.moon);
        } else if (isLunarEclipse) {
            this.moon.setTint(0xff7700);
            this.showEclipsePopup('📸Gerhana Bulan terjadi!');
        } else {
            this.clearEclipseEffect();
            this.moon.clearTint();
        }
    }
    showEclipsePopup(message) {
        const popupX = window.innerWidth / 2;
        const popupY = window.innerHeight - 100;
        const popup = this.add.text(popupX, popupY, message, {
            fontSize: '20px', 
            fill: '#fff', 
            backgroundColor: '#000', 
            padding: { x: 20, y: 10 },
        ...
        const popupLine = this.add.graphics();
        popupLine.lineStyle(2, 0xffffff, 0.8);
        popupLine.strokeLineShape(new Phaser.Geom.Line(popupX, popupY - 20, this.moon.x, this.moon.y));
